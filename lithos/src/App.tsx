...
function RevealLayer({ image, cursorX, cursorY }: RevealLayerProps) {
  ...
  // Repaint mask on every render (cursorX/Y change drives this)
  useEffect(() => {
    const canvas = canvasRef.current;
  ...
  return (
    ...
    >
        ...
        <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 bg-white/20 backdrop-blur-md border border-white/30 rounded-full px-2 py-2 items-center gap-1">
          {NAV_ITEMS.map((item, i) => (
            <button
              ...
              className={
                i === 0
                  ? 'px-4 py-1.5 rounded-full text-sm font-medium bg-white text-gray-900'
      ...
      >
        ...
        {/* 2 — Reveal layer (z-30, handled inside RevealLayer) */}
        ...
        {/* 4 — Bottom-left paragraph (z-50, hidden on mobile) */}
        <div
